...
const steps = [
	{
		...
		icon: Camera,
		color: 'bg-blue-500/20 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400',
	},
	{
		title: 'Analyze',
		description: 'AI recognizes and interprets the mathematical notation',
		icon: ScanSearch,
		color: 'bg-purple-500/20 text-purple-600 dark:bg-purple-500/10 dark:text-purple-400',
	},
	{
		title: 'Formalize',
		description: 'Problem is converted to formal mathematical language',
		icon: FileCode,
		color: 'bg-orange-500/20 text-orange-600 dark:bg-orange-500/10 dark:text-orange-400',
	},
	{
		title: 'Verify',
		description: 'Solution is proven with formal verification methods',
		icon: CheckCircle2,
		color: 'bg-emerald-500/20 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400',
	},
];
...
export function WorkflowSection() {
	...
	useEffect(() => {
		...
		if (!section || stepElements.length === 0 || typeof window === 'undefined') return;
		...
		if (connector) {
			tl.fromTo(connector, { height: 0 }, { height: '100%', duration: 1.5, ease: 'power3.out' }, 0);
		}

		// Animate each step
		stepElements.forEach((step, index) => {
			tl.fromTo(step, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.5, ease: 'back.out(1.7)' }, index * 0.2);
		});
			...
			<div className="container max-w-6xl">
				<div className="text-center max-w-3xl mx-auto mb-16">
					...
					<p className="text-muted-foreground text-lg">
						Our streamlined process transforms your math problems into formally verified solutions
					</p>
				</div>
				...
				<div className="relative max-w-3xl mx-auto">
					...
					<div className="space-y-16 md:space-y-24 relative">
						{steps.map((step, index) => (
							...
							>
								<div className={`flex-none z-10 flex items-center justify-center h-14 w-14 rounded-full ${step.color}`}>
