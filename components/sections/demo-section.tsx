...
const examples = [
	{
		id: 'algebra',
		name: 'Algebra',
		problem: 'Prove that for any integers a, b, and c, if a divides bc and gcd(a, b) = 1, then a divides c.',
		solution: [
	...
	},
	{
		id: 'number-theory',
		name: 'Number Theory',
		problem: 'Prove that the sum of two consecutive perfect squares cannot be a perfect square.',
		solution: [
...
export function DemoSection() {
	...
	const activeSolution = examples.find((ex) => ex.id === activeTab)?.solution || [];
			...
			<div className="container max-w-6xl">
				<div className="text-center max-w-3xl mx-auto mb-16">
					...
					<p className="text-muted-foreground text-lg">
						Explore examples of how our platform solves different types of mathematical problems
					</p>
				</div>
				...
				<div className="bg-card border rounded-xl shadow-lg overflow-hidden">
					<Tabs defaultValue="algebra" value={activeTab} onValueChange={setActiveTab}>
						...
						{examples.map((example) => (
							<TabsContent key={example.id} value={example.id} className="p-6 space-y-8">
								<div>
								...
								<div>
									<div className="flex items-center justify-between mb-3">
										...
										<Button variant="outline" size="sm" onClick={handlePlayDemo} disabled={isPlaying}>
											<Play className="h-4 w-4 mr-2" />
											Watch Solution
										</Button>
									</div>

									<div className="bg-background border rounded-lg p-4 h-[300px] overflow-y-auto" ref={solutionRef}>
										<div className="space-y-4">
											{example.solution.slice(0, currentStep).map((step, index) => (
												<motion.div
													key={index}
