...
interface FileUploaderProps {
	onUploadComplete: (imageFile: File) => void;
}
					...
					<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
						<div className="relative overflow-hidden rounded-lg border">
							<div className="absolute top-2 right-2 z-10">
								<Button variant="destructive" size="icon" onClick={removeFile} className="h-7 w-7 rounded-full">
									<X className="h-4 w-4" />
								</Button>
							</div>
							...
							<div className="p-3 bg-muted/50 border-t">
								<div className="flex items-center justify-between">
									...
									<span className="text-xs text-muted-foreground">{(files[0]?.size / 1024 / 1024).toFixed(2)} MB</span>
								</div>
							</div>
						</div>
