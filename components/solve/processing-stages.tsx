...
const stages: Record<
  Exclude<ProcessingState, "idle" | "complete">,
  StageInfo
> = {
  uploading: {
    icon: FileImage,
    ...
    <div className="relative py-8">
      ...
      <div className="grid md:grid-cols-4 gap-4">
        {Object.entries(stages).map(([stage, info], index) => {
          const stageEnum = stage as Exclude<
            ProcessingState,
            "idle" | "complete"
          >;
          const isActive = currentStage === stageEnum;
          const isComplete =
            getStageIndex(currentStage) > getStageIndex(stageEnum);
          ...
          return (
            ...
            >
              ...
              <h3
                className={`font-medium text-center ${isActive ? "text-primary" : ""}`}
              >
                {info.label}
              </h3>
