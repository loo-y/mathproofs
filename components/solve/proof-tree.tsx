...
export function ProofTree({ proofTree }: ProofTreeProps) {
	...
	useEffect(() => {
		...
		gsap.fromTo(labels, { opacity: 0 }, { opacity: 1, duration: 0.5, stagger: 0.1, delay: 1 });
	}, []);
	...
	return (
		<svg ref={svgRef} width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} className="max-w-full max-h-full">
			{/* Edges */}
			...
			{proofTree.nodes.map((node) => {
				...
				return (
					<g key={node.id} transform={`translate(${position.x}, ${position.y})`}>
						<circle r={nodeRadius} fill="currentColor" fillOpacity="0.1" stroke="currentColor" strokeWidth="2" className="node" />
						<foreignObject x={-90} y={-10} width="180" height="20" className="node-label">
